...
local hovered = 0    -- month dropdown: the row under the pointer, 0 = none
...
-- The dropdown's hover bar.  Marking a group is left to the caller: a move
-- between rows repaints the bar alone, while opening, closing and switching
-- page repaint the whole list anyway.
local function setHover(month, group)
    hovered = month
    set('MLMonth', tostring(month), group)
end

-- The pointer entered row m (1..12) of the dropdown.
function HoverMonth(m)
    setHover(m, 'MonthBar')
    flush()
end

-- The pointer left row m.  Crossing between rows fires the entered row's
-- MouseOverAction and the left row's MouseLeaveAction in an order Rainmeter
-- does not promise, so a leave is only allowed to clear the bar while it still
-- owns it.  Without that test, every crossing in whichever direction reports
-- the leave second would wipe the highlight the over had just set.
function UnhoverMonth(m)
    if hovered ~= m then return end
    setHover(0, 'MonthBar')
    flush()
end
...
function ToggleMonthList()
    set('MLHide', variable('MLHide', '1') == '1' and '0' or '1', 'MonthList')
    setHover(0)
    flush()
end
...
function Initialize()
    ...
    viewYear, viewMonth = nil, nil
    selYear, selMonth, selDay = nil, nil, nil
    hovered, live, ready = 0, nil, false
